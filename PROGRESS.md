...
Implementation notes
- 2026-03-22: Added shared transient retry helpers and typed offline/retryable request errors to both mobile and web client API layers, then taught session bootstrap/auth restore to reuse cached completion state instead of clearing the session on temporary network loss.
- 2026-03-22: Added a durable backend `client_mutations` ledger plus `Idempotency-Key` handling on intent/profile/global-rules/agent respond endpoints so replayed core mutations return cached results instead of duplicating work. Mobile/web outbox and UX follow-through (`CR-02` to `CR-04`) remain separate tracked work.
