...
Minimum read depth:
- For `PROGRESS.md`: read fully.
- For other docs: read headings first, then open full sections needed for the active milestone.

## 2. Resume Implementation From PROGRESS
Treat `PROGRESS.md` as source of truth.

Select the next actionable work item:
```bash
awk '/^- \[( |~)\]/{print NR ":" $0; exit}' PROGRESS.md
```
...
5. Update `PROGRESS.md` checkboxes and add a dated note under `Implementation Notes`.
...
- If a decision is ambiguous, make a reasonable choice and record it in `PROGRESS.md` before coding.
