...
| `profile-promotion` readiness-pack group | `9/10` | New named group expands to `profile-persistence-current`, `profile-preferences-current`, `profile-preferences-reopen-current`, `profile-photo-current`, `other-profile-current`, and `chats-thread-current` | Needs `pnpm test:mobile:readiness-pack -- --lane=profile-promotion` to pass in the current release window before grouped Profile rows can move to `10/10` |
| `settings-protocol-promotion` readiness-pack group | `9/10` | New named group expands to `settings-current`, `settings-reopen-current`, and `settings-photo-current` for Settings identity, reopen, media, and protocol visibility summaries | Needs `pnpm test:mobile:readiness-pack -- --lane=settings-protocol-promotion` to pass in the current release window, plus backend/SDK protocol proof for full protocol operations |
