...
| Protocol integrations panel | `9/10` | Panel now reads one backend/SDK `protocol/visibility-summary` read model through mobile React Query and shows linked app details with stable selectors | Needs richer action coverage and a fresh local Maestro rerun |
