...
| Profile overview | `9/10` | Current-state local lane now passes through `profile-screen` and both edit sections; `profile-promotion` now groups the broader same-window proof | Needs the profile promotion group to pass in the current release window before it can be claimed as `10/10` |
| Profile bio persistence | `9/10` | Current-state local lane passed on 2026-04-19 through bio/location save assertion; `profile-promotion` pairs it with preferences, reopen, avatar, and peer-profile coverage | Needs the profile promotion group to pass in the current release window before it can be claimed as `10/10` |
| Profile interests persistence | `9/10` | Current-state local lane passed on 2026-04-19 through interests save assertion; `profile-promotion` pairs it with preferences and persisted-reopen coverage | Needs the profile promotion group to pass in the current release window before it can be claimed as `10/10` |
...
| Settings shell | `9/10` | Current-state local Maestro lane reaches `settings-screen` cleanly; `settings-protocol-promotion` groups the same-window Settings proof | Needs the settings/protocol promotion group to pass in the current release window before it can be claimed as `10/10` |
| Settings identity persistence | `9/10` | Current-state local reopen lane passed on 2026-04-20 through save + close + reopen assertions; `settings-protocol-promotion` pairs it with protocol panel and media coverage | Needs the settings/protocol promotion group to pass in the current release window before it can be claimed as `10/10` |
| Protocol integrations panel | `9/10` | Settings Maestro lanes now include stable assertions for `settings-protocol-panel`, linked apps, access summary, and delivery queue summary coverage | Needs `settings-protocol-promotion` plus protocol backend/SDK proof for action/operation management before it can be claimed as `10/10` |
| Other user profile | `9/10` | Current-state local peer-profile lane passed on 2026-04-20 through open + action-shell + close; `profile-promotion` includes the peer-profile lane and chat-thread modal provenance coverage | Needs the profile promotion group to pass in the current release window before it can be claimed as `10/10` |
...
| `profile-promotion` readiness-pack group | `9/10` | Named group expands to `profile-persistence-current`, `profile-preferences-current`, `profile-preferences-reopen-current`, `profile-photo-current`, `other-profile-current`, and `chats-thread-current`; `--list` now shows the group evidence, promoted rows, and score-honesty note without running anything | Needs `pnpm test:mobile:readiness-pack -- --lane=profile-promotion` to pass in the current release window before grouped Profile rows can move to `10/10` |
| `settings-protocol-promotion` readiness-pack group | `9/10` | Named group expands to `settings-current`, `settings-reopen-current`, and `settings-photo-current`; `--list` now shows the group evidence, promoted rows, protocol caveat, and score-honesty note without running anything | Needs `pnpm test:mobile:readiness-pack -- --lane=settings-protocol-promotion` to pass in the current release window, plus backend/SDK protocol proof for full protocol operations |
...
| `test:mobile:readiness-pack` | `9/10` | Root runner now groups the remaining MVP promotion lanes for auth/onboarding/Home recovery, Profile, settings/protocol, chat thread, chat mutations, peer profile, and notifications, with auth landing available as an optional signed-out lane; `--list` is dry-only and prints actionable promotion evidence/notes for the named groups | Needs one successful local run before the gated rows can be promoted to `10/10`; use `--lane=profile-promotion,settings-protocol-promotion` for the focused Profile plus Settings/protocol same-window proof |
...
- Most improved this pass: route graph proof, top-layer E2E navigation, mutually exclusive transient route state, profile/settings photo update proof, the reproducible mobile readiness pack, protected auth landing selectors for the designed video/cycling-title screen, a named auth/onboarding/Home recovery promotion lane, and named Profile plus Settings/protocol promotion groups
